...
This job generate spend forecast for the future 24 months base on spend data.

Currently it works with the forecast model ARMA, ARIMA, SARIMA and Prophet.
...
Note: For the moment, this job only run forecast for the top 10 active combinations, due to the long computation time needed for SARIMA model hyperparameter search (around 10 minutes per each combination).
This behaviour can be changed easily in the related python file.
...
Related files: [`pipeline/jobs/save_as_csv_for_powerbi.py`]

This job save a copy the latest version of historic spend data and forecast as `csv` format to blob storage. It is meant for easy consumption from other tools e.g. PowerBI.

Currently it fetch the historic spend data and forecast with the most recent creation date from blob storage, then convert them to `csv` format, finally saving them to a pre-defined file path on blob storage.

This jobs requires the `SpendDataFilledMissingMonth` and  `ForecastOutput` data file in blob storage, which is generated by the jobs above.
The output is stored on blob storage in folder `DataForPowerBI`.

##### To run this job on a local machine:
```shell
make pipeline_save_as_csv
```
