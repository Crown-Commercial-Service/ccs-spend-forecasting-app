...
#### 2. Run forecast and output to blob

Related files: [`pipeline/jobs/run_forecast_and_output_to_blob.py`]

This job generate spend forecast for the future 12 months base on spend data.

Currently it works with the forecast model ARMA / ARIMA and SARIMA.
It pulls data from the blob storage, compares different models performance for each Category/MarketSector combination, then for each combination use the better performed model to generate forecast, finally output the forecast to blob storage as .csv file.

Note: For the moment, this job only run forecast for several pre-defined combinations, due to the long computation time needed for running SARIMA model (around 10 minutes per each combination).
This behaviour can be changed easily in the related python file

This jobs requires the `SpendDataFilledMissingMonth` data file in blob storage, which is generated by the job above.
