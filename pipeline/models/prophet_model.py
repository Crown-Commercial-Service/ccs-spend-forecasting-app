...
class ProphetModel(ForecastModel):
    ...
    def rename_columns_for_input(self, input_df: pd.DataFrame) -> pd.DataFrame:
        ...
        return input_df.rename(
            columns={self.amount_column: "y", self.date_column: "ds"}
        )
    ...
    def find_best_prior_scales(
        ...
        current_combination = tuple(
            category_sector_spend.iloc[0][self.columns_to_consider]
        )

        self.logger.debug(
            f"Finding best prior scales for combination: {current_combination}"
        )
        ...
        search_range = product(
            changepoint_prior_scale_values, seasonality_prior_scale_values
        )
        ...
        all_mape = pd.DataFrame(
            data=all_mape,
            columns=["changepoint_prior_scale", "seasonality_prior_scale", "mape"],
        ).sort_values("mape", ascending=True)
        best_params = tuple(
            all_mape.iloc[0][["changepoint_prior_scale", "seasonality_prior_scale"]]
        )
