...
class ArmaModel(SarimaModel):
    ...
    Args:
        name (str):
            Name of the model instance. Default to be "ARMA"
    ...
    def full_model_name(self, order: tuple[int, int, int]) -> str:
        """Return the name of this model with given set of hyperparameters. For reduce hard-coding for logging."""
        p, q = order[0], order[2]
        return f"{self.name}({p, q})"

    def hyperparameter_range(self) -> dict:
        """Return a default set of search range for hyperparameters.
        For ARMA model, the order of differencing (d) is confined to be 0.
        Also, the seasonal order (P,Q,D,m) are confined to be 0.
        Returns:
            dict:
        """

        return {
            "p": range(0, 4, 1),  # p, order of AR part
            "q": range(0, 4, 1),  # q, order of MA part
            "d": 0,  # d
            "seasonal_P": [0],  # P
            "seasonal_Q": [0],  # Q
            "seasonal_D": 0,  # D
